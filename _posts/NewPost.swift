...
var fileText = "---\ntitle: \"\(title)\"\ndate: \(year)-\(month)-\(day)\n---\n\n"
fileText += "<h2><a href=\"http://evandekhayser.com/\(year)/\(month)/\(day)/\(hyphenSeparatedTitle)\" class=\"title\">\(title)</a></h2>\n<h5>\(day) \(monthStrings[Int(month)!]!) \(year)</h5>\n\n"
let currentPath = FileManager.default.currentDirectoryPath
print(currentPath)
let path = URL.init(fileURLWithPath: currentPath + "/\(year)-\(month)-\(day)-\(hyphenSeparatedTitle).md")
